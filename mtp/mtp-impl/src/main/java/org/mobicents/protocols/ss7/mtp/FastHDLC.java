...
public class FastHDLC {
    ...
    public static final int RETURN_NO_SYNC_FLAG = 0x8000;
    ...
    public static final int STATUS_MASK = 0x8000;
    public static final int STATUS_VALID = 0x8000;
    public static final int STATUS_CONTROL = 0x0000;
    public static final int ONES_MASK = 0x7000;
    public static final int DATA_MASK = 0x00ff;
     ...
     */
    private int[][] hdlc_encode = new int[5][256];
                ...
                if ((c & 0x0200) != 0) {
                    ...
                    if ((c & 0x0100) == 0) {
                        if (bits == 6)
                            ...
                            return HFP(STATUS_CONTROL, 0, consumed + 1, CONTROL_COMPLETE);
                        return HFP(STATUS_CONTROL, 0, consumed, CONTROL_ABORT);
                    }
                    /*
                     * Yuck! It's something else... Abort this entire frame, and start looking for a good frame
                     */
                    return HFP(STATUS_CONTROL, 0, consumed + 1, CONTROL_ABORT);
                }
    ...
    private int hdlc_encode_precalc(int x, int y) {
        ...
        return (data << (32 - bits)) | (ones << 8) | bits;
    }
    ...
    public void fasthdlc_init(HdlcState h) {
        ...
        h.state = h.bits = h.data = h.ones = 0;
    }

    public int fasthdlc_tx_load_nocheck(HdlcState h, int c) {
        int res;
        res = hdlc_encode[h.ones][c & 0xff];
        h.ones = (res >>> 8) & 0x0f;
    ...
    public int fasthdlc_rx_run(HdlcState h) {
        int next;
        if (h.bits < minbits[h.state])
            return RETURN_EMPTY_FLAG;
        while (bit_count < 8) {
            /*
            ...
            case PROCESS_FRAME:
                ...
                case STATUS_CONTROL:
                    if ((next & DATA_MASK) == CONTROL_COMPLETE)
                        /* A complete, valid frame received */
                        return RETURN_COMPLETE_FLAG;
                    h.state = FRAME_SEARCH;
                    bit_count = 0;
                    /* An abort (either out of sync or explicit) */
                    return RETURN_DISCARD_FLAG;
                case STATUS_VALID:
                    return next & DATA_MASK;
                }
            }
        }
        bit_count -= 8;
        return RETURN_NO_SYNC_FLAG;
    }
}
